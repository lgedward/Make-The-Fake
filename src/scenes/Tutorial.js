class Tutorial extends Phaser.Scene {
    ...
    create() {
        ...
        this.anims.create({
            ...
            frameRate: 10,
            repeat: -1 // This animation plays once and stops at the last frame
        });    
        this.anims.create({
            key: 'explosion',
        ...
        this.displayTutorialStep1();
    }

    displayTutorialStep1() {
        let textPositionX = game.config.width / 50;
        let textPositionY = game.config.height / 3;
        
        let message1 = "Press up arrow during jumps to \ndo a flip! Be careful to stick the \nlanding or you'll be roadkill! \nPress C to continue";
        this.messageText = this.add.text(textPositionX, textPositionY, message1, {
            fontSize: '20px',
            fill: '#FFF',
            align: 'center'
        }).setOrigin(0.5);
        ...
        this.input.keyboard.once('keydown-C', () => {
            this.messageText.destroy(); // Remove the first message
            this.displayTutorialStep2();
        });
    }

    displayTutorialStep2() {
        let textPositionX = game.config.width / 50;
        let textPositionY = game.config.height / 3;
        
        let message2 = "You get points when you successfully \ndo a flip. Press C to start";
        this.messageText = this.add.text(textPositionX, textPositionY, message2, {
            fontSize: '20px',
            fill: '#FFF',
            align: 'center'
        }).setOrigin(0.5);
        ...
        this.input.keyboard.once('keydown-C', () => {
            this.messageText.destroy(); // Remove the second message
            this.startGame();
        });
    }
